...
#define JETSAM_PRIORITY_IDLE_DEFERRED                                          \
  1 /* Keeping this around till all xnu_quick_tests can be moved away from     \
       it.*/
...
#define MEMORYSTATUS_FLAGS_SNAPSHOT_COPY                                       \
  0x4 /* Returns the previously populated snapshot created by the system */
...
#define MEMORYSTATUS_FLAGS_SNAPSHOT_DROP_OWNERSHIP                             \
 ...
 *early candidate for jetsam when the device is under memory pressure. Non-fatal
 *limit types are the
...
#define P_MEMSTAT_MANAGED                                                      \
  0x00000800 /* Process is managed by assertiond i.e. is either application or \
                extension */
...
extern int memorystatus_update(proc_t p, int priority, uint64_t user_data,
                               boolean_t is_assertion, boolean_t effective,
...
void memorystatus_get_task_page_counts(task_t task, uint32_t *footprint,
                                       ...
                                       uint32_t *purgeable_pages);
void memorystatus_invalidate_idle_demotion_locked(proc_t p,
                                                  boolean_t clean_state);
void memorystatus_update_priority_locked(proc_t p, int priority,
